...
const updateBook = async function (req, res) {
    try {
        ...
        let updatedBook = await bookModel.findOneAndUpdate({ _id: bookId },{$set: bookData}, { new: true });
...
const deletedBook = async function (req, res) {
    try {
        ...
        if (!data) return res.status(400).send({msg: "id required"})
        let dBook = await bookModel.findOneAndUpdate({ _id: data }, { $set: { isDeleted: true, deletedAt: Date() } }, { new: true })
        if (!dBook) return res.status(400).send({ msg: "id is wrong" })
